...
import { DatabaseConnection, InviteConnection, InviteeConnection, ProjectorConnection } from './database';
import { AuthorConnection } from './database';
import { GuestbookConnection } from './database';
import { PhotoConnection } from './database';
...
class TempStore {
    ...
    photos: Photo[] = [];
    invitees: Invitee[] = [];
    projector: Projector = new Projector('home', '', 30_000, false);
...
class TempPhotoConnection implements PhotoConnection {
    ...
    async getAll(professional: boolean = false): Promise<Photo[]> {
        return tempStore.photos.filter(p => p.professional === professional);
    }
    ...
    async create(name: string, mimeType: string, data: Buffer, professional: boolean = false, caption?: string): Promise<Photo> {
        const photo = new Photo(tempStore.createUuid(), name, mimeType, professional, caption);
...
class TempProjectorConnection implements ProjectorConnection {
    async get(): Promise<Projector> {
        return new Projector(
            tempStore.projector.mode, 
            tempStore.projector.message, 
            tempStore.projector.dwellMs, 
            tempStore.projector.paused);
    }
    async setMode(mode: ProjectorMode): Promise<void> {
        tempStore.projector.mode = mode;
    }
    async setMessage(message: string): Promise<void> {
        tempStore.projector.message = message;
    }
    async setDwellMs(dwellMs: number): Promise<void> {
        tempStore.projector.dwellMs = dwellMs;
    }
    async getGuestbookEntryIds(): Promise<string[]> {
        return tempStore.guestbook.filter(e => e.visible && !e.moderated).map(e => e.id);
    }
}

class TempDatabaseConnection implements DatabaseConnection {
    invites: InviteConnection = new TempInviteConnection();
    authors: AuthorConnection = new TempAuthorConnection();
    guestbook: GuestbookConnection = new TempGuestbookConnection();
    photos: PhotoConnection = new TempPhotoConnection();
    invitees: InviteeConnection = new TempInviteeConnection();
    projector: ProjectorConnection = new TempProjectorConnection();

    private isInitialized = false;
    /** Ensures only one init runs if many handlers call `database()` at once */
    private initPromise: Promise<this> | null = null;

    async init(): Promise<DatabaseConnection> {
...
export const database = new TempDatabaseConnection();
...
class DummyData {
    ...
    async createInvites(db: TempDatabaseConnection) {
    ...
    async createGuestbookEntries(db: TempDatabaseConnection) {
